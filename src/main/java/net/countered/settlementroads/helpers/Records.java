...
public class Records {
    ...
    public record RoadData(int width, String roadTypeId, List<BlockState> materials, List<RoadSegmentPlacement> roadSegmentList) {
        public static final Codec<RoadData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                ...
                Codec.STRING.fieldOf("road_type_id").forGetter(RoadData::roadTypeId),
    ...
    public record StructureConnection(BlockPos from, BlockPos to, ConnectionStatus status, float progress) {
        public static final Codec<StructureConnection> CODEC = RecordCodecBuilder.create(instance ->
                instance.group(
                        ...
                        Codec.FLOAT.optionalFieldOf("progress", 0.0f).forGetter(StructureConnection::progress)
                ).apply(instance, StructureConnection::new)
        );
        
        // 兼容旧版本的构造函数
        public StructureConnection(BlockPos from, BlockPos to) {
            this(from, to, ConnectionStatus.PLANNED, 0.0f);
        }
        
        public StructureConnection(BlockPos from, BlockPos to, ConnectionStatus status) {
            this(from, to, status, 0.0f);
        }
        
        // 更新进度的方法
        public StructureConnection withProgress(float newProgress) {
            return new StructureConnection(from, to, status, Math.max(0.0f, Math.min(1.0f, newProgress)));
        }
        
        // 更新状态的方法
        public StructureConnection withStatus(ConnectionStatus newStatus) {
            return new StructureConnection(from, to, newStatus, progress);
        }
    }
