...
public class RoadPathCalculator {
    ...
    public interface ProgressCallback {
        void onProgress(float progress, int currentSteps, int maxSteps, String status);
    }
    ...
    private static int getNeighborDistance() {
        try {
            return ConfigManager.getInstance().getPathFindingConfig().getNeighborDistance();
        } catch (Exception e) {
            return 4;
        }
    }
    
    private static double getDiagonalStepCost() {
        try {
            return ConfigManager.getInstance().getPathFindingConfig().getDiagonalStepCost();
        } catch (Exception e) {
            return 1.5;
        }
    }
    
    private static double getStraightStepCost() {
        try {
            return ConfigManager.getInstance().getPathFindingConfig().getStraightStepCost();
        } catch (Exception e) {
            return 1.0;
        }
    }
    ...
    private static int getMaxSteps() {
        try {
            return ConfigManager.getInstance().getPathFindingConfig().getMaxSteps();
        } catch (Exception e) {
            return 1000;
        }
    }
    
    private static double getHeuristicWeight() {
        try {
            return ConfigManager.getInstance().getPathFindingConfig().getHeuristicWeight();
        } catch (Exception e) {
            return 1.0;
        }
    }
    
    private static double getCostMultiplier(String type, double defaultValue) {
        try {
            double multiplier = ConfigManager.getInstance().getPathFindingConfig().getCostMultiplier(type);
            return multiplier != 1.0 ? multiplier : defaultValue;
        } catch (Exception e) {
            return defaultValue;
        }
    }
    ...
    public static List<Records.RoadSegmentPlacement> calculateAStarRoadPath(
        ...
        return calculateAStarRoadPath(start, end, width, serverWorld, null);
    }
    
    public static List<Records.RoadSegmentPlacement> calculateAStarRoadPath(
            BlockPos start, BlockPos end, int width, ServerWorld serverWorld, ProgressCallback progressCallback
    ) {
        long startTime = System.currentTimeMillis();
        LOGGER.debug("Starting pathfinding from {} to {}", start, end);
        ...
        int neighborDistance = getNeighborDistance();
        ...
        int maxStepsConfig = getMaxSteps();
        int totalSteps = maxStepsConfig;
        int currentStep = 0;
        
        while (!openSet.isEmpty() && maxStepsConfig-- > 0) {
            currentStep++;
            Node current = openSet.poll();

            if (progressCallback != null && currentStep % 100 == 0) {
                float progress = (float) currentStep / totalSteps;
                float distanceProgress = 1.0f - (float) current.pos.withY(0).getManhattanDistance(endGround.withY(0)) / 
                    (float) startGround.withY(0).getManhattanDistance(endGround.withY(0));
                float combinedProgress = Math.min(progress * 0.7f + distanceProgress * 0.3f, 0.95f);
                
                progressCallback.onProgress(combinedProgress, currentStep, totalSteps, "Searching path...");
            }

            if (current.pos.withY(0).getManhattanDistance(endGround.withY(0)) < neighborDistance * 2) {
                long endTime = System.currentTimeMillis();
                LOGGER.debug("Found path! {} in {}ms ({} steps)", current.pos, endTime - startTime, getMaxSteps() - maxStepsConfig);
                
                if (progressCallback != null) {
                    progressCallback.onProgress(1.0f, currentStep, totalSteps, "Path calculation completed");
                }
            ...
            for (int[] offset : neighborOffsets) {
                ...
                double stepCost = (offsetSum == 2 * neighborDistance) ? getDiagonalStepCost() : getStraightStepCost();
                ...
                double tentativeG = current.gScore + stepCost
                        + elevation * getCostMultiplier("elevation", 40.0)
                        + biomeCost * getCostMultiplier("biome", 8.0)
                        + yLevelCost * getCostMultiplier("seaLevel", 8.0)
                        + terrainStabilityCost * getCostMultiplier("terrainStability", 16.0);
        ...
        long endTime = System.currentTimeMillis();
        LOGGER.warn("No path found from {} to {} in {}ms ({} steps)", start, end, endTime - startTime, getMaxSteps());
    ...
    private static double heuristic(BlockPos a, BlockPos b) {
        ...
        return dxzApprox * getHeuristicWeight();
    }
